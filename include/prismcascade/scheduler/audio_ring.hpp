...
class audio_ring {
   public:
    ...
    std::size_t         write(const double* src, std::size_t n, memory::timestamp_t ts);
    std::size_t         read(double* dst, std::size_t n);
    std::size_t         size() const noexcept { return len_; }
    memory::timestamp_t tail_stamp() const noexcept { return tail_stamp_; }

   private:
    std::vector<double> buf_;
    std::size_t         cap_, head_{0}, tail_{0}, len_{0};
    memory::timestamp_t tail_stamp_{0};
};
}  // namespace prismcascade::scheduler
