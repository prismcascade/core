/* ── tests/12_test_transform.cpp ───────────────────────────────
   unit + property tests for ast::internal_transform::assign / cut / detach_cross_edges
   (gTest + RapidCheck)
 *───────────────────────────────────────────────────────────── */
...
TEST(TransformAssign, ParentAndBackrefsUpdate) {
    ...
    auto diff = ast::internal_transform::assign(P, 0, C);
...
TEST(TransformAssign, SubEdgeBackrefUpdate) {
    ...
    auto diff = ast::internal_transform::assign(B, 0, ast::SubEdge{A, 0});
...
TEST(TransformCut, CutChildRemovesParent) {
    ...
    auto diff = ast::internal_transform::cut(P, 0);
    EXPECT_FALSE(inputIsChild(P, 0, C));
    EXPECT_TRUE(C->parent.expired());

    /* redo */
    for (auto& st : diff) ast::internal_transform::assign(st.node, st.index, st.old_value);
...
TEST(TransformCut, CutSubEdgeRemovesBackref) {
    ...
    auto B = makeNode(31);
    B->inputs.resize(1, std::monostate{});
    ast::internal_transform::assign(B, 0, ast::SubEdge{A, 0});  // back-ref auto-created

    auto diff = ast::internal_transform::cut(B, 0);
    EXPECT_FALSE(hasBackRef(A, 0, B, 0));
...
TEST(TransformDetach, InnerOuterEdgesBothDirections) {
    ...
    ast::internal_transform::assign(X, 0, ast::SubEdge{A, 0});  // A → X

    auto steps = ast::internal_transform::detach_cross_edges(A);
...
RC_GTEST_PROP(TransformProp, AssignIsInvolution, ()) {
    ...
    auto d1 = ast::internal_transform::assign(P, 0, v1);
...
TEST(TransformCut, CrossEdgeSubtreeDetach) {
    ...
    ast::internal_transform::assign(Y, 0, ast::SubEdge{A, 0});

    /* inner → outer */
    Z->inputs.resize(1, std::monostate{});
    ast::internal_transform::assign(Z, 0, ast::SubEdge{A, 0});

    ast::internal_transform::detach_cross_edges(A);
...
TEST(DetachNoTouch, InnerToInnerKept) {
    ...
    ast::internal_transform::assign(A, 0, B);  // parent(B)=A
    B->inputs.resize(1, std::monostate{});
    ast::internal_transform::assign(B, 0, C);  // parent(C)=B

    /* ── 内→内 SubEdge:  A[0] → C.slot0 ─── */
    C->inputs.resize(1, std::monostate{});
    ast::internal_transform::assign(C, 0, ast::SubEdge{A, 0});  // back-ref 自動生成
...
TEST(DetachNoTouch, OuterToOuterKept) {
    ...
    Y->inputs.resize(1, std::monostate{});
    ast::internal_transform::assign(Y, 0, ast::SubEdge{X, 0});  // X → Y

    auto Z = makeNode(313);  // unrelated subtree
    ast::internal_transform::detach_cross_edges(Z);
...
TEST(DetachUndo, FullRoundTrip) {
    ...
    /* A → B  (main-tree) */
    A->inputs.resize(1, std::monostate{});
    ast::internal_transform::assign(A, 0, B);  // parent linkは assign が設定

    /* X → A  (cross SubEdge) */
    X->inputs.resize(1, std::monostate{});
    ast::internal_transform::assign(X, 0, ast::SubEdge{A, 0});
...
TEST(AssignSubEdgeToSubEdge, BackrefSwitch) {
    ...
    C->inputs.resize(1, std::monostate{});
    ast::internal_transform::assign(C, 0, ast::SubEdge{A, 0});
    EXPECT_TRUE(hasBackRef(A, 0, C, 0));
    EXPECT_FALSE(hasBackRef(B, 0, C, 0));

    ast::internal_transform::assign(C, 0, ast::SubEdge{B, 0});
...
TEST(AssignError, SlotOutOfRange) {
    auto N = makeNode(400);
    EXPECT_THROW(ast::internal_transform::assign(N, 3, 42), std::domain_error);
}
...
TEST(AssignError, MainTreeCycle) {
    ...
    P->inputs.resize(1, std::monostate{});
    C->inputs.resize(1, std::monostate{});
    ast::internal_transform::assign(P, 0, C);                                   // P → C
    EXPECT_THROW(ast::internal_transform::assign(C, 0, P), std::domain_error);  // would create cycle
}

TEST(CutError, EmptySlot) {
    auto N = makeNode(410);
    N->inputs.resize(1, std::monostate{});
    EXPECT_THROW(ast::internal_transform::cut(N, 0), std::domain_error);
}

TEST(DetachError, Nullptr) {
    std::shared_ptr<ast::AstNode> nullNode;
    EXPECT_THROW(ast::internal_transform::detach_cross_edges(nullNode), std::domain_error);
}
...
TEST(TransformExtended, AssignDifferentSlotDoesNotAffectOthers) {
    ...
    ASSERT_NO_THROW(ast::internal_transform::assign(P, 1, C1));
    ASSERT_NO_THROW(ast::internal_transform::assign(P, 2, 42));

    ASSERT_NO_THROW(ast::internal_transform::assign(P, 1, C2));  // slot-1 を差し替え
    ...
    for (int step = 0; step < kSteps; ++step) {
        ...
        try {
            if (op == 0) {
                ...
                if (kind == 0) {
                    ast::internal_transform::assign(nodes[n_idx], slot, nodes[*rc::gen::inRange(0, kNodes)]);
                } else if (kind == 1) {
                    auto from = nodes[*rc::gen::inRange(0, kNodes)];
                    ast::internal_transform::assign(nodes[n_idx], slot, ast::SubEdge{from, 0});
                } else {
                    ast::internal_transform::assign(nodes[n_idx], slot, *rc::gen::arbitrary<std::int64_t>());
                }
            } else {
                ast::internal_transform::cut(nodes[n_idx], slot);
            }
