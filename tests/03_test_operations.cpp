...
TEST(Cut, Cut_Node_UndoRedo) {
    ...
    auto d = ast::internal_transform::cut(P, 0);  // remove C
    EXPECT_FALSE(has_parent(C, P));
    undo(d);
    EXPECT_TRUE(has_parent(C, P));
    redo(d);
...
TEST(Cut, Cut_SubEdge_UndoRedo) {
    ...
    auto d = ast::internal_transform::cut(X, 0);
    EXPECT_FALSE(has_backref(A, 0, X, 0));
    undo(d);
    EXPECT_TRUE(has_backref(A, 0, X, 0));
}
...
TEST(Detach, Detach_KeepOuterOuter) {
    ...
    auto steps = ast::internal_transform::detach_cross_edges(Z);
...
TEST(Errors, Error_DetachNull) {
    ...
    EXPECT_THROW(ast::internal_transform::detach_cross_edges(np), std::domain_error);
}
...
TEST(OperationMoveDeep, TripleNestMoveUndoRedo) {
    ...
    /* redo → また R 直下へ */
    for (auto& st : diff) ast::internal_transform::assign(st.node, st.index, st.new_value);
    EXPECT_TRUE(inputIsNode(R, 0, C));
}
